...
const EditUser = () => {
  ...
  const handleEditUser = (e) => {
    ...
    if (form?.image?.files.length > 0) {
      ...
      fetch(`https://aide-server-gray.vercel.app/edit/${user?._id}`, {
        ...
        body: JSON.stringify(userDitail),
      })
        .then((res) => res.json())
        .then((result) => {
          console.log(result);
  ...
  return (
    <div>
      <div className="mb-10">
        <h1 className="text-[#975EFE] text-3xl">Edit {user?.name}</h1>
      </div>
      <>
        <div>
          <div className="flex flex-no-wrap items-start">
            <div className="w-full ">
              <form onSubmit={handleEditUser} className=" px-2">
                <div className="bg-white rounded shadow  py-3">
                  ...
                  <button
                    type="submit"
                    className="bg-purple-700 md:ml-4 rounded hover:bg-purple-600 transform duration-300 ease-in-out text-sm font-medium px-6 py-4 text-white lg:max-w-[144px] w-full "
                  >
                    Edit User
                  </button>
                </div>
